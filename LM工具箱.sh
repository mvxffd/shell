...
GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw/refs/heads/main/LM%E5%B7%A5%E5%85%B7%E7%AE%B1.sh"
# 国内代理前缀（如无代理可留空）
...
check_update() {
    ...
    echo -e "${C_BG2} ${F_GRAY}下载地址: ${F_WHITE}$download_url${RESET}"
...
do_update() {
    ...
    if curl -sS --max-time 60 --fail -o "$tmp_file" "$download_url" 2>/dev/null; then
        ...
        # ============================================
        # 【关键修复】下载后自动修正第17行和第19行
        # ============================================
        echo -e "${C_BG2} ${F_YELLOW}🔧 正在检查并修复脚本配置...${RESET}"
        
        # 1. 如果第17行包含了代理前缀+github.com，修正为正确的格式（保留github.com格式）
        if grep -q 'GITHUB_RAW_URL="https://gh.kejilion.pro/https://github.com/mvxffd/shell/raw' "$tmp_file" 2>/dev/null; then
            echo -e "${C_BG2} ${F_YELLOW}⚠️ 检测到URL包含代理前缀，正在修复...${RESET}"
            sed -i 's|GITHUB_RAW_URL="https://gh.kejilion.pro/https://github.com/mvxffd/shell/raw|GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw|g' "$tmp_file"
        fi
        
        # 2. 如果第17行被改成了raw.githubusercontent.com，强制改回github.com格式
        if grep -q 'GITHUB_RAW_URL="https://raw.githubusercontent.com/mvxffd/shell/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"' "$tmp_file" 2>/dev/null; then
            echo -e "${C_BG2} ${F_YELLOW}⚠️ 检测到URL被改为raw格式，正在修复为github.com格式...${RESET}"
            sed -i 's|GITHUB_RAW_URL="https://raw.githubusercontent.com/mvxffd/shell/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"|GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"|g' "$tmp_file"
        fi
        
        # 3. 如果第17行是raw.githubusercontent.com的其他变体，也强制改回github.com
        if grep -q 'GITHUB_RAW_URL="https://raw.githubusercontent.com' "$tmp_file" 2>/dev/null; then
            echo -e "${C_BG2} ${F_YELLOW}⚠️ 检测到URL为raw格式，正在修复为github.com格式...${RESET}"
            sed -i 's|GITHUB_RAW_URL="https://raw.githubusercontent.com/mvxffd/shell/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"|GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"|g' "$tmp_file"
            sed -i 's|GITHUB_RAW_URL="https://raw.githubusercontent.com/mvxffd/shell/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"|GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88.sh"|g' "$tmp_file"
        fi
        
        # 4. 确保 GH_PROXY 是干净的代理前缀
        if grep -q 'GH_PROXY="https://gh.kejilion.pro/https://' "$tmp_file" 2>/dev/null; then
            echo -e "${C_BG2} ${F_YELLOW}⚠️ 检测到GH_PROXY格式异常，正在修复...${RESET}"
            sed -i 's|GH_PROXY="https://gh.kejilion.pro/https://|GH_PROXY="https://gh.kejilion.pro/|g' "$tmp_file"
        fi
        
        echo -e "${C_BG2} ${F_GREEN}✅ 脚本配置检查完成${RESET}"
