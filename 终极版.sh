...
GITHUB_RAW_URL="https://raw.githubusercontent.com/mvxffd/shell/refs/heads/main/%E7%BB%88%E6%9E%81%E7%89%88%E6%9C%AC.sh"
# 国内代理前缀（如无代理可留空）
...
do_update() {
    ...
    if curl -sS --max-time 60 --fail -o "$tmp_file" "$download_url" 2>/dev/null; then
        ...
        # 1. 如果第17行包含了代理前缀，自动修正为直连URL
        if grep -q 'GITHUB_RAW_URL="https://gh.kejilion.pro/https://' "$tmp_file" 2>/dev/null; then
            echo -e "${C_BG2} ${F_YELLOW}⚠️ 检测到URL包含代理前缀，正在修复...${RESET}"
            sed -i 's|GITHUB_RAW_URL="https://gh.kejilion.pro/https://raw.githubusercontent.com|GITHUB_RAW_URL="https://raw.githubusercontent.com|g' "$tmp_file"
        fi
        
        # 2. 如果第17行是 github.com 而不是 raw.githubusercontent.com，也修正
        if grep -q 'GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw' "$tmp_file" 2>/dev/null; then
            echo -e "${C_BG2} ${F_YELLOW}⚠️ 检测到GitHub地址格式不正确，正在修复...${RESET}"
            sed -i 's|GITHUB_RAW_URL="https://github.com/mvxffd/shell/raw|GITHUB_RAW_URL="https://raw.githubusercontent.com/mvxffd/shell/refs/heads|g' "$tmp_file"
        fi
        
        # 3. 确保 GH_PROXY 是干净的代理前缀（不会被重复拼接）
        ...
        # 4. 如果 GH_PROXY 为空，保持原样
        # 5. 如果 GH_PROXY 不为空且第17行是 raw 地址，保持原样（运行时动态拼接）
